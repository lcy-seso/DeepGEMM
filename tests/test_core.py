...
def per_token_cast_to_fp8(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    x = (
        torch.nn.functional.pad(x, (0, pad_size), value=0)
        if pad_size > 0
        else x
    )
...
def construct_contiguous_grouped(
    ...
    y_fp8 = (
        ...
        torch.empty(
            (num_groups, ceil_div(n, 128), k // 128),
            device="cuda",
            dtype=torch.float,
        ),
    )
...
def construct_masked_grouped(
    ...
    x_fp8 = (
        ...
        torch.empty(
            (num_groups, max_m, k // 128), device="cuda", dtype=torch.float
        ),
    )
    y_fp8 = (
        torch.empty_like(y, dtype=torch.float8_e4m3fn),
...
def construct_k_grouped_wgrad(m: int, n: int, k_sizes: List[int]) -> Tuple[
    ...
    x_scales = torch.empty(
        (total_scale_factors, m), device="cuda", dtype=torch.float
    )
    y_scales = torch.empty(
        (total_scale_factors, n), device="cuda", dtype=torch.float
    )
    ...
    for k in k_sizes:
        ...
        x_scales[scale_offset : scale_offset + num_scales].copy_(
            x_scale_chunk.T
        )
        y_scales[scale_offset : scale_offset + num_scales].copy_(
            y_scale_chunk.T
        )
...
def test_m_grouped_gemm_contiguous() -> None:
    ...
    for num_groups, expected_m_per_group, k, n in (
        ...
        out = torch.where(
            (m_indices == -1).unsqueeze(1), torch.zeros_like(out), out
        )
        diff = calc_diff(out, ref_out)
        assert diff < 0.001, f"{m=}, {k=}, {n=}, {diff:.5f}"
...
def test_wgrad_gemm():
    ...
    for k in (4096, 8192):
        for m, n in (
            ...
            t = bench_kineto(
                test_func, "fp8_wgrad_gemm", suppress_kineto_output=True
            )
            print(
