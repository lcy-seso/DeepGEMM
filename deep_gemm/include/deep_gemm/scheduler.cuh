...
template <GemmType kGemmType, uint32_t SHAPE_N, uint32_t BLOCK_M,
          uint32_t BLOCK_N, uint32_t kNumGroups, uint32_t kNumTMAMulticast,
...
struct Scheduler {
  ...
  __device__ __forceinline__ void get_swizzled_block_idx(
      const uint32_t& num_m_blocks, const uint32_t& block_idx,
