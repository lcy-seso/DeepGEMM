...
template <uint32_t SHAPE_M, uint32_t SHAPE_N, uint32_t BLOCK_M,
          uint32_t BLOCK_N, uint32_t BLOCK_K, uint32_t kNumStages,
          uint32_t kNumLastStages, uint32_t kNumTMAThreads,
          uint32_t kNumMathThreadsPerGroup, uint32_t kNumTMAMulticast,
...
__global__ void __launch_bounds__(
  ...
  auto scheduler = Scheduler<GemmType::Normal, SHAPE_N, BLOCK_M, BLOCK_N, 1,
                             kNumTMAMulticast, kIsTMAMulticastOnA>(SHAPE_M);
  ...
  #pragma unroll
          for (uint32_t s = 0; s < kNumInnerStages; ++s) {
            ...
            tma_copy(&tensor_map_scales_b,
                     reinterpret_cast<uint64_t*>(&full_barrier),
                     smem_scales_b[s], n_block_idx * BLOCK_N, k_idx / BLOCK_K,
                     num_tma_multicast_b);
  ...
  #pragma unroll
        for (auto i = 0; i < WGMMA::kNumAccum / 4; ++i) {
          ...
          st_shared(smem_d_0 + i * 4, {d_0.x + shifted_accum[i * 4 + 0],
                                       d_0.y + shifted_accum[i * 4 + 1]});
          float2 d_1 = ld_shared(smem_d_1 + i * 4);
          st_shared(smem_d_1 + i * 4, {d_1.x + shifted_accum[i * 4 + 2],
      ...
      if (threadIdx.x == 0) {
        cute::SM90_TMA_STORE_2D::copy(&tensor_map_d, smem_d,
                                      n_block_idx * BLOCK_N,
                                      m_block_idx * BLOCK_M);
